...
* **Specification**: Code implementation MUST stand firmly on BOTH a valid `implementation_map_v<version>.md` (located under `agent-workspace/plans/<feature-name>/implementation_maps/`) AND a Test Plan (`phase-5-verification.md` or feature test specifications). Source code modification without both prerequisites is strictly prohibited.
...
* **Goal**: Confirm alignment with test specifications in `phase-5-verification.md` and critical system feature assertions before code scaffolding.
* **Auto-Detection Scanning Rule**:
  * Read `agent-workspace/plans/<feature-name>/phase-5-verification.md`. Check for test suites, unit test contracts, and regression assertions.
* **Reframed Grill Prompt**:
...
* **Reframed Grill Prompt**:
  ...
  > * Test Plan: `phase-5-verification.md`
