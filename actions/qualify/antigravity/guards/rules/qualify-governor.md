...
* **A provisional certification is strictly prohibited from unlocking the `/release` action.**
...
2. **Release Gating**: If all release criteria are met and certification is `full`, the agent updates `agent-workspace/plans/<feature-name>/PROCESS_STATUS.md` marking Row 5 (`/qualify`) as `Completed` and unlocks `/release`. If defects exist or certification is `provisional`, Row 5 remains blocked.
