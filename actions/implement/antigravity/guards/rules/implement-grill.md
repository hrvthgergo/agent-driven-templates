...
2. **Baseline 2: Strict Directory Separation & Production Cleanliness**: Source code in `codebase-*`, test harnesses in `codebase-qualify/src/`, AST code graphs in `agent-workspace/src/<layer>/code_graph/`, and system documentation in `agent-workspace/docs/`.
...
* **Q4b: Test Harness Construction Ordering (Red-First vs. Feature-First)**: Determine when the cross-layer Test Harness stream (`codebase-qualify/`) is built relative to feature code:
  ...
  4. Other / Free-text
* **Q5: Sequential vs. Parallel Stream Execution Order**: Confirm sequencing of decoupled parallel execution tasks, including the Test Harness stream (`codebase-qualify/`).
...
* **Q8: Sub-Repository / Multi-Repo Symlink Integrity Verification**: Run symlink resolution check on `agent-workspace/src/` (including `qualify`).
