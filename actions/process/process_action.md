...
This document defines the requirements, design decisions, and step-by-step specification for the `/process` action. This action is a dedicated, standalone lifecycle element designed to process historical codebases, analyze legacy documentation, and execute codebase refactoring or restructuring without cluttering the `/init` action.
...
> [!IMPORTANT]
> **No Code Rewriting**: `/process` is strictly an organizational and migration workflow. It MUST NOT rewrite, refactor, or modify existing source code or logic. Code logic transformations are out of scope for `/process` and are deferred entirely to the `/plan` and `/implement` actions.

### The Three Knowledge Inputs
The `/process` action synthesizes three distinct knowledge sources to construct the restructuring proposal and updated blueprints:
...
Execution follows an 8-node state machine (Nodes S0 through S7), adhering to the **Action Context Notification Law (Combined Multi-Layer Strategy)** (prefixing every response turn with `> 📍 **Active Workflow**: /process | **Scope**: <branch> | **Node**: <Node_ID>`, printing node transition badges, and maintaining disk header metadata):
...
| Command | Description |
|:---|:---|
| `/process` (or `/process --plan`) | **Plan-First Mode** (Default). Runs Q&A grill, generates `restructure-proposal.md`, and pauses for explicit developer consent before modifying disk state |
| `/process --auto` (or `/process --apply`) | **Immediate Execution Mode**. Runs Q&A grill, executes intact file migration to `codebase-*` sub-repos immediately, and records execution audit log |
| `/process --dry-run` | Performs historical analysis and outputs proposed migration report without moving any files |
| `/process --docs-only` | Extracts documentation and synthesizes phase blueprints without proposing physical file restructuring |
...
| `/process --full-sync` | **Full Synchronization Mode**. Executes core migration, Code Graph generation, and system documentation update in one pass |
