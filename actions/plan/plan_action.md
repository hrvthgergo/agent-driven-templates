...
This document serves as the authoritative baseline specification for the `/plan` action in the **Guards Framework**. It governs how AI agents interactively create, maintain, and structure token-optimized, agent-ready 6-phase blueprint documentation, research reports, and version-linked implementation maps before code execution begins.

---

## 1. General Introduction & Core Philosophy

The `/plan` action is the architectural bridge between environment setup (`/init` / `/process`) and code implementation (`/implement`). It transforms raw ideas, feature requirements, and historical context into structured, unambiguous Phase Blueprints, research summaries, decision records, and structured implementation roadmaps.
...
- **Universal Design Specifications (Tier 2 - Platform-Agnostic)**:
  - [summary.md](file:///Users/horvathgergo/Desktop/agent-driven-templates/actions/summary.md): Central framework sitemap and operational lifecycle.
  - [grill_engine.md](file:///Users/horvathgergo/Desktop/agent-driven-templates/actions/grill_engine.md): Interactive Q&A interview engine rules.
  - [process_handling.md](file:///Users/horvathgergo/Desktop/agent-driven-templates/actions/process_handling.md): Process status matrix and release governance rules.
  - [code_graph_taxonomy.md](file:///Users/horvathgergo/Desktop/agent-driven-templates/actions/code_graph_taxonomy.md): Code graph structure and taxonomy specification.
  - [implementation_map_taxonomy.md](file:///Users/horvathgergo/Desktop/agent-driven-templates/actions/implementation_map_taxonomy.md): Implementation map structure and schema specification.
  - [plan_action.md](file:///Users/horvathgergo/Desktop/agent-driven-templates/actions/plan/plan_action.md) (*This Document*): Core `/plan` state machine design, boundary rules, and execution reasoning.
  - [plan_questions.md](file:///Users/horvathgergo/Desktop/agent-driven-templates/actions/plan/plan_questions.md): Interactive planning questionnaire schema and scanning rules.
...
1. **Structured Document & Version Naming**: An implementation map is a formal document adhering to [implementation_map_taxonomy.md](file:///Users/horvathgergo/Desktop/agent-driven-templates/actions/implementation_map_taxonomy.md). It is named after the target software version (e.g. `implementation_map_v1.0.0.md` or `implementation_map_v1.1.0_engine.md`).
2. **Step-by-Step Agent Guidance**: Serves as the agent's authoritative roadmap during the `/implement` action.
...
- **STRICT PROHIBITION**: **ONLY the creation/drafting of the `implementation_map.md` document is allowed during `/plan`. NO code scaffolding, file editing in `src/` or `codebase-*/`, or actual code implementation is permitted during `/plan`!** Source code implementation remains strictly reserved for the `/implement` action.
...
* **Description**: Immediately follows the initial summary. Executes stateful Q&A interview governed by [grill_engine.md](file:///Users/horvathgergo/Desktop/agent-driven-templates/actions/grill_engine.md).
* **Primary Goals**:
...
| Command | Description |
|:---|:---|
| `/plan` | Default interactive planning mode (synthesizes feature understanding, conducts Q&A grill, and generates phase blueprints) |
| `/plan --feature <feature_name>` | Explicitly targets planning for a specific feature branch scope |
| `/plan --auto` | Automatic planning mode (evaluates system impact and scaffolds phase blueprints using default parameters) |
