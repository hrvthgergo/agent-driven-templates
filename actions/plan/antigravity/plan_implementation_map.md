...
*   **Antigravity Implementation Guideline**: This document (`plan_implementation_map.md`) is the concrete execution roadmap detailing how our agent implements those baselines within **Google Antigravity** using its native environment primitives:
    ...
    *   **Rules (`rules/`)**: Permanent constraint instructions enforcing unchangeable baselines, verification and operations authority bounds, prompting laws, and the sequential Q1–Q11 (with Q10.1–Q10.5) interview schema (e.g. `plan-grill.md`).
    *   **Skills (`skills/`)**: Specialized capability packages providing procedures and scripts for 6-phase blueprint scaffolding (including the 8-section `phase-6-operation.md` content contract), `phase_details/` subfolder creation, research report generation, project-durable `TEST_STRATEGY.md` authoring/amending, scenario authoring/ratification (`agent-workspace/tests/scenarios/SC-<feature-slug>-<nnn>.md`), and versioned implementation map drafting (e.g. `plan-generator/SKILL.md`).
...
| **6-Phase Blueprint Architecture** | [plan_action.md](file:///Users/horvathgergo/Desktop/agent-driven-templates/actions/plan/plan_action.md#L137-L181) | Template & Skill Primitives | `templates/phase-*.md` (Phase 1–6) & `skills/plan-generator/SKILL.md` |
...
| **Operations Design Authority & Content Contract (§0–§7)** | [plan_action.md](file:///Users/horvathgergo/Desktop/agent-driven-templates/actions/plan/plan_action.md#L154-L260) & [plan_questions.md](file:///Users/horvathgergo/Desktop/agent-driven-templates/actions/plan/plan_questions.md#L220-L274) | Rule, Skill & Template Primitives | `rules/plan-grill.md` (Baseline 9), `skills/plan-generator/SKILL.md`, `templates/phase-6-operation.md` |
| **Sequential Q1–Q11 (with Q10.1–Q10.5) Grill Schema** | [plan_questions.md](file:///Users/horvathgergo/Desktop/agent-driven-templates/actions/plan/plan_questions.md#L52-L290) | Rule & Workflow Primitives | `rules/plan-grill.md` & `workflows/plan.md` |
...
*   **List of Actions**:
    ...
    2. Define YAML frontmatter (`name: plan`, `description: Interactive planning workflow for 6-phase blueprints, test strategies, scenarios, operations design, and versioned implementation maps`).
...
*   **List of Actions**:
    ...
    2. Encode **Unchangeable Baseline Enforcement**:
        ...
        *   *Baseline 9 (Operations Design Authority Guard)*: Establishes `/plan` as the sole design authority for operations (§0–§7 of `phase-6-operation.md`). Specifies, never constructs (no Dockerfile, compose file, pipeline YAML, deploy script). Names only for secrets — never values. Observability contracts in §6c are never authored as `SC-*` scenarios.
        *   *Baseline 10 (First-Definer Rule)*: The first feature to define an environment (§0) or monitoring tool (§6b) owns the canonical row; later features reference it and record deltas.
    3. Encode Prompting Laws (neutral choices, free-text option on every question, max-2-questions-per-turn rule, no `[Recommended]` tags).
    4. Encode full sequential questionnaire schema (Q1 to Q11) matching [plan_questions.md](file:///Users/horvathgergo/Desktop/agent-driven-templates/actions/plan/plan_questions.md):
        ...
        *   Q10: Phase 6 - Operations & Environment Design (Q10.1 Environment Topology, Q10.2 Containerization & Image Impact, Q10.3 Configuration & Secret Declarations, Q10.4 CI/CD Pipeline Impact & Promotion Policy, Q10.5 Observability & Monitoring Design)
...
*   **List of Actions**:
    1. Create `actions/plan/antigravity/guards/templates/PROCESS_STATUS.md`: Pre-configured matrix template with sub-rows 3.1–3.6 for 6 planning phases and versioned implementation map rows, referencing `/operate` for Row 6.
    2. Create `actions/plan/antigravity/guards/templates/GRILL_STATUS.md`: Stateful transcript log template for `/plan` interview sessions (including Q10.1–Q10.5 records).
    ...
    5. Create `actions/plan/antigravity/guards/templates/phase-1-summary.md` through `phase-5-test.md`: Comprehensive starter blueprint templates featuring embedded decision sections, research link placeholders, system impact analysis, and verification references.
    6. Create `actions/plan/antigravity/guards/templates/phase-6-operation.md`: Complete 8-section content contract starter template (§0 Environment Topology matrix, §1 Containerization, §2 Service Orchestration, §3 Config/Secrets, §4 CI/CD Pipeline, §5 Delivery/Promotion + Post-Delivery Hooks, §6 Observability [6a signals, 6b tooling, 6c health/readiness/alert contracts], §7 ADRs).
    7. Create `actions/plan/antigravity/guards/templates/implementation_map.md`: Starter implementation map template adhering to 5-block taxonomy schema with version header.
